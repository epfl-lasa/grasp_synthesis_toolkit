...
% * F: Finger with joints moved to desired q
    ...
    if isequal(eval_type,'sym')
        ...
        try
            ...
            % finger.joints_pos = double(subs(finger.symbolic.joints_pos, Q, q.'));
            % finger.tip = double(subs(finger.symbolic.tip, Q, q.'));
            ...
            for k = 1:numel(finger.Link) % iterate over links
                ...
                % finger.Link{k}.HT_this = double(subs(finger.Link{k}.symbolic.HT_this, Q, q.'));
                % finger.Link{k}.HT_next = double(subs(finger.Link{k}.symbolic.HT_next, Q, q.'));
