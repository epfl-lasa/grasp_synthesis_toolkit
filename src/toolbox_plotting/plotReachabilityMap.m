...
if nargin < 2
    data = load('reachable_map.mat');
    hand_map = data.map_hand;
end
if nargin < 3
    ...
    if isa(map_finger,'struct') % palm is a struct. For fingers, it is cell array of links.
        ...
        for l = 1:nlink-1 % number of real links (ignore fingertip)
            if ismember(l,finger_links) || any(isnan(finger_links))
                ...
                shp = alphaShape(px,py,pz,20); % Inf: produces the convex hull
                plot(shp,'Facecolor',cmap(cidx,:),'EdgeColor',cmap(cidx,:),'EdgeAlpha',0.15,'FaceAlpha',0.25);
                cidx = cidx + 1; % for generating color
                hold on;
