function [rmap, hand] = rMapActiveLinks(hand, fngrs2spl, if_plot, if_save)
...
%     the robot model
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nf = hand.n; % number of fingers to sample
...
if nargin < 3
    if_plot = false; % if visualize the reachability map after constructing it
end
if nargin < 4
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rmap = cell(1,hand.n+1); % the (nf+1)th rmap is the map of palm
for f = 1:nf % iterate over each finger
    % disp('--------------------------------------------------');
    fprintf('Sampling finger: %d\n', f);
    finger = hand.F{f};
    nl = finger.nlink; % nl contains fingertip
    nd = finger.n;     % DoF of the f-th finger (1 less than nl)

    map_finger = cell(1,nl); % last map is the fingertip
    
    if (~fngrs2spl(f)) || (~hand.factv(f))
        fprintf('Finger %d is not active or not moving. Store current position.\n', f);
        ...
        fprintf("Finger %d is active. Sample over the possible range.\n", f);
    ...
    for l = 1:finger.nlink-1 % Only for all real and virtual links except fingertip
        ...
        if (finger.idx_real_link(l) && isempty(k)) || (~finger.idx_real_link(l)) % only for real link
            ...
            fprintf('Finger %d Link %d: has no boundary: %d, is a real link: %d\n', f, l, isempty(k), finger.idx_real_link(l));
            %fprintf('* isempty(k): %d\n', isempty(k));
        ...
        if nargout > 1
            hand.reachability_map = true; % flag to indicate that all rmaps have been added to link field
            hand.F{f}.Link{l}.map_link = map_link; % update robot model by adding the rmap to the link
        end
    end
    rmap{f} = map_finger; % update the link of the hand
end
...
basepoints_xy = hand.T\hand.P.basepoints_h; % (4,N), project palm vertices back on XY plane (before multiply T)
...
palmmesh_h = hand.P.HTctr2inr*(hand.T*palmmesh_xyz_h.'); % first project back to palm plane, then project from palm to palm inner surface
palmmesh = palmmesh_h(1:3,:).'; % (N,3)
rmap{nf+1} = struct('idx', 0, 'linkmesh', palmmesh);
% disp('--------------------------------------------------');
