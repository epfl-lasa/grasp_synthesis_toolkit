function object = compObject(Param,solMu)
...
if ~isfield(Param,'res') % default resolution
    Param.res = 20;
end
if ~isfield(Param,'transl')
    Param.t = [0;0;0]; % zero translation
end
if ~isfield(Param,'quat') % default quaternion: no rotation
    Param.quat = quaternion(0,0,0,1);
end
if ~isfield(Param,'height') % default height
    Param.height = 10;
end
if ~isfield(Param,'radius') % default radius
    Param.rad = 3;
end
h = Param.height;
rad = Param.radius;
...
%%% add sphere centers to the object
if size(Param.sphereCenter,1)~=length(Param.sphereRadius)
    warning("Object parameters for center and radius have incompatible size\n")
end
sphCtr = Param.sphereCenter; % sphereCenter is (N x 3)
sphCtr = rotatepoint(quat,sphCtr).' + center; % store in sphCtr as (3 x N)
...
object.relSphereCenter = Param.sphereCenter;
object.sphereCenter = sphCtr;             % (3 x N) Array
object.sphereRadius = Param.sphereRadius; % (1 x N) Array

object.ctr = center;   
object.quat = quat;         % quaternion to rotate coordinates OF to WF
...
if nargin >1 
    ...
    object.cp = [ center + rotatepoint(quat, [0,0,h*solMu(1)]).',...
                    center + rotatepoint(quat, [0,0,h*solMu(2)]).'];
    object.mu = solMu;
end
%% symbolic expressions (used for constraint description)
object.sym.ctr = symCtr;% center of the object
object.sym.quat = symQuat;         % q1,q2,q3,q4 (for optimization)
object.sym.n = symNormal;  % axis of the cylinder
object.sym.axisPtArray = symAxPtArray;        % (3 x n) coordinates of equidistant spheres
